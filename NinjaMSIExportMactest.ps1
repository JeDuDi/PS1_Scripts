...
# file paths    
$MSI_export = "C:\MSIExports\" + $today + "_Ninja_MSI_Export_Mac.csv"   
...
Foreach ($organization in $organizations) {
    ...
    Foreach ($location in $locations) {
        ...
        $MSI_url = "https://app.ninjarmm.com/v2/organization/" + $organizationID + "/location/" + $locationID + "/installer/MAC_PKG"
        $MSI = Invoke-RestMethod -Uri $MSI_url -Method GET -Headers $headers
        $MSIOutput = "C:\MSIExports\" + $organization.Name + $location.Name + ".pkg"
        ...
        #create bat file
        $BATexport = "C:\MSIExports\" + $organization.Name + $location.Name + ".txt"
        $BATcode = 'installer -pkg ' + $msi.url + ' -target /Applications'
